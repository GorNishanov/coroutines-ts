...
deduced from \tcode{return},\added{ \tcode{coroutine-return-keyword}, and \tcode{yield-keyword}} statements in the body of the function, if any.
\end{quote}
...
multiple \tcode{return}\added{, \tcode{coroutine-return-keyword}, and \tcode{yield-keyword}} statements, the return type is deduced for each
\tcode{return}\added{, \linebreak
\tcode{coroutine-return-keyword}, and \tcode{yield-keyword}} statement. If the type deduced is not the same in each
...
%
%\pnum
%If a coroutine has a declared return type that contains a placeholder type has multiple \tcode{yield-keyword}/\tcode{coreturn} statements, the return type is deduced for each
%\tcode{yield-keyword}/\tcode{coreturn} statement. If the type deduced is not the same in each
%deduction, the program is ill-formed.
...
\begin{itemize}
\item If a \tcode{yield-keyword} statement and an \grammarterm{await-expression} are present, then
the return type is \tcode{std::experimental::async_stream<T>}, where \tcode{T} is deduced from the \tcode{yield-keyword} statements as if a \tcode{yield-keyword} statement were a \tcode{return} statement in a function with declared type \tcode{auto} without a \grammarterm{trailing-return-type}.
...
\tcode{coroutine-return-keyword} statements as if a \tcode{coroutine-return-keyword} statement were a \tcode{return} statement in a function with declared type \tcode{auto} without a \grammarterm{trailing-return-type}.

%NOTE: as if 

\item Otherwise, if a \tcode{yield-keyword} statement is present in a function, then the return type is \linebreak
\tcode{std::experimental::generator<T>},
where \tcode{T} is deduced from the \tcode{yield-keyword} statements  as if a \tcode{yield-keyword} statement were a \tcode{return} statement in a function with declared type \tcode{auto} without a \grammarterm{trailing-return-type}.
