...
%  \item If the \grammarterm{cast-expression} is a prvalue, $a$ is a temporary object copy-initialized from the \grammarterm{cast-expression}, otherwise $a$ is an lvalue referring to the result of evaluating the \grammarterm{cast-expression}.
  ...
  \item $a$ is \cbstart\added{the} \cbend \grammarterm{cast-expression} if 
...
\item \cbstart \removed{If for an invocation of the operator \tcode{co_await} applied to expression $a$ at least one viable operator \tcode{co_await} candidate is visible 
%\cxxref{over.match.oper},
(through unqualified name lookup (\cxxref{basic.lookup.unqual}) 
or \linebreak 
argument-dependent-lookup (\cxxref{basic.lookup.argdep}))
then $o$ is the result of that invocation (which shall be well-formed); otherwise $o$ is $a$.}

\added{$o$ is the function call expression }\tcode{\added{operator co_await(\textit{a})}} \added{if overload resolution for that function call finds at least one viable function; otherwise, $o$ is $a$.} 
\cbend
%
...
%  \tcode{await_ready}, \tcode{await_suspend} and \tcode{await_resume} are looked up in the scope of class $A$ as if by class member access lookup (3.4.5) and if this lookup finds at least one declaration, then, $o$ is $a$; otherwise, $o$ is \tcode{operator co_await($a$)}.
  \item \cbstart\removed{If $o$ is a prvalue,} $e$ is a temporary object copy-initialized from $o$ \added{if $o$ is a prvalue;} otherwise $e$ is an lvalue referring to the result of evaluating $o$. \cbend 
...
\begin{itemize}
  \item If the result is \tcode{false}, the coroutine is considered suspended. Then, the \textit{await-suspend} expression is evaluated. If that expression has
  \cbstart \added{non-\tcode{void} type, then it has}
  type \tcode{bool} and \added{if the value is} \removed{returns} \tcode{false}, the coroutine is resumed. \cbend
  If that expression exits via an exception, the exception is caught, the coroutine is resumed, and the exception is immediately re-thrown (15.1). Otherwise, control flow returns to the current caller or resumer (\ref{dcl.fct.def.coroutine}) \cbstart \added{without exiting any scopes (\ref{stmt.jump})}\cbend.
