...
or if await-suspend-expr type is not void, it is equivalent to
...
\grammarterm{}{cast-expression}, and \textit{await-ready-expr}, \textit{await-suspend-expr} and \textit{await-expr} are
...
\begin{itemize}
	...
	\tcode{await_ready}, \tcode{await_suspend} and \tcode{await_resume} are
	are \tcode{__expr.await_ready()}, \tcode{__expr.await_suspend(coroutine-handle)} and \tcode{__expr.await_resume()},
	respectively;
...
\rSec3[expr.await.awaitable]{Awaitable expression}
