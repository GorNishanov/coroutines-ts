...
\pnum
Let \textit{f} be an \tcode{operator await} function found according to the rules of operator function lookup for unary operators and \tcode{operator await}(\ref{over.await}) for \textit{cast-expression}. If such function cannot be found, the program is ill-formed. 
...
of the enclosing coroutine, \textit{P} be the type of the promise object, \textit{h} be an object of \tcode{std::experimental::coroutine_handle<\textit{P}>} referring to the enclosing coroutine, and let \textit{await-ready-expr}, \textit{await-suspend-expr}, and \textit{await-resume-expr} be
...
\pnum
A coroutine is considered suspended just prior to the call to \textit{await-suspend-expr}. If a return type of \textit{await-suspend-expr} is \tcode{bool} and invocation of \textit{await-suspend-expr} returns \tcode{false}, then execution continues as if coroutine was resumed. An exception thrown from within \textit{await-suspend-expr} is propagated as if coroutine was resumed and an exception was thrown just prior to an invocation of \textit{await-resume-expr}.
