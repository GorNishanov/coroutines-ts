...
%In this section add the \tcode{await_kw} \grammarterm{cast-expression} 
...
\begin{bnf}
	...
	\terminal{-{-}} cast-expression\br
%	\added{\terminal{await_kw} cast-expression}\br
...
\begin{quote}
\pnum
The \tcode{await_kw} operator is used to suspend evaluation of the enclosing coroutine (\ref{dcl.fct.def.coroutine}) while awaiting completion of the computation represented by the operand expression.

\begin{bnf}
  \nontermdef{await-expression}\br
  \terminal{await_kw} cast-expression
\end{bnf}
...
%
%A potentially-evaluated \tcode{await_kw} expression shall not appear in a
