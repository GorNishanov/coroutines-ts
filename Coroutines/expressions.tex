...
If the type of \textit{await-suspend-expr} is \textit{cv-void}, then
...
where \textit{suspend-resume-point}s are suspend-resume-points as defined in (\ref{dcl.fct.def.coroutine}).
