...
%Let $e$ be a temporary initialized as-if by \tcode{auto\&\& e = \textit{f(cast-expression)}}, if there is no viable function, then $e$ is a \textit{cast-expression}. 
%and the cast expression has class type X, the unqualified-ids \tcode{await_ready}, \tcode{await_suspend}, and \tcode{await_resume} are looked up in the scope of class X
%as if by class member access lookup (\ref{basic.lookup.classref}), and if any of these three lookups finds at least one declaration then e is cast-expression.

%\pnum
%Let \textit{f} be an \tcode{operator co_await} function found according to the rules of operator function lookup for unary operators and \tcode{operator co_await}(\ref{over.await}) for \textit{cast-expression}. If such a function cannot be found, the program is ill-formed. 
%Let $e$ be a temporary initialized as-if by \tcode{auto\&\& e = \textit{f(cast-expression)}}; <snip move up> 
%Let \textit{p} be the promise object (\ref{dcl.fct.def.coroutine})
