...
\begin{itemize}
	...
	\tcode{await_ready(__expr)}, \tcode{await_resume(__expr, _ResumableHandle)} and \tcode{await_resume(__expr)} 
	respectively, where 
	\tcode{await_ready}, \tcode{await_suspend} and \tcode{await_resume} are 
...
\pnum
An \tcode{await} expression may appears as an unevaluated operand (\cxxref{expr.typeid}, \cxxref{expr.sizeof}, \cxxref{expr.unary.noexcept}, \cxxref{dcl.type.simple}). A presence of such await expression does not make enclosing function resumable and can be used to examine the type of an await expression.
...
\pnum
An \tcode{await} expression may only appear in the resumable functions 
that have eventual return type, i.e. have \tcode{set_result} member
function defined in a \term{promise type} (\ref{dcl.fct.def.resumable}) of the enclosing resumable function.
