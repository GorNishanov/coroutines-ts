...
\pnum
The \tcode{await} operator is used to suspend evaluation of the enclosing coroutine function (\ref{dcl.fct.def.coroutine}) while awaiting
for completion of the computation represented by the operand expression.

\pnum
The presence of an \tcode{await} operator in a potentially-evaluated expression makes the enclosing function a coroutine function.
...
\pnum
An \tcode{await} expression may only appear in a coroutine function 
