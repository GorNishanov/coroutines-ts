...
\pnum
  Let \textit{e} be the operand of the \grammarterm{yield-expression} and \textit{p} be the promise object of the enclosing coroutine, then the \grammarterm{yield-expression} is equivalent to the expression \tcode{co_await} \textit{p}\tcode{.yield_value}(\textit{e}). A \grammarterm{yield-expression} shall only appear within a suspension context of a function (\ref{expr.await}).
