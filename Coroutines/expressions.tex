...
\item \cbstart\added{$o$ is $a$, unless} \removed{If} \cbend for an invocation of the operator \tcode{co_await} applied to expression $a$ at least one viable operator \tcode{co_await} candidate is visible 
...
then $o$ is the result of that invocation (which shall be well-formed)\cbstart\removed{; otherwise $o$ is $a$}\cbend.
%
