...
\pnum
\cbstart
Evaluation of an \grammarterm{await-expression} involves the following auxiliary \added{types, } expressions\added{, and objects}:
\cbend
...
%  \item If the \grammarterm{cast-expression} is a prvalue, $a$ is a temporary object copy-initialized from the \grammarterm{cast-expression}, otherwise $a$ is an lvalue referring to the result of evaluating the \grammarterm{cast-expression}.
  ...
  \removed{an \grammarterm{await-expression} is an implicit one} \added{the \grammarterm{await-expression} was implicitly} produced by a \grammarterm{yield-expression} (\ref{expr.yield}), or an initial, or final suspend points (\ref{dcl.fct.def.coroutine}).
  \cbend
