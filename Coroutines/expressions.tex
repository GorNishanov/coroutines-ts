...
%NOTES: 5.3.8
%Fix indentation
%Editing instructions inside sections
...
% NOTES: Put there restriction on
% noexcept

%\pnum
%The presence of an \tcode{coawait} operator in a potentially-evaluated expression makes the enclosing function a coroutine.
...
A potentially-evaluated await expression shall not appear in a default argument (\cxxref{dcl.fct.default}).

%a\pnum
%A \tcode{coawait} expression shall not appear in a potentially-evaluated expression in a \grammarterm{handler} (\cxxref{except.handle}).
%A \tcode{coawait} expression shall not appear in an \grammarterm{initializer-clause} of a default parameter (\cxxref{dcl.fct.default}).

%\rSec2[expr.await.evaluation]{Evaluation of await operator}

%\pnum
%Let \textit{T} be the type of the \grammarterm{cast-expression} and \textit{p} be the promise object (\ref{dcl.fct.def.coroutine})
...
\enternote
An \tcode{coawait} expression may appear as an unevaluated operand (\cxxref{expr.typeid}, \cxxref{expr.sizeof}, \cxxref{expr.unary.noexcept}, \cxxref{dcl.type.simple}). The presence of such an \tcode{coawait} expression does not make the enclosing function a coroutine and can be used to examine the type of an \tcode{coawait} expression.
...
\pnum
An \tcode{coawait} expression may only appear in a coroutine 
