...
%	{
%		auto && __range = range-init;
%		for ( auto __begin = await_kw begin-expr,
%		__end = end-expr;
%		__begin != __end;
%		await_kw ++__begin ) {
%			@\textit{for-range-declaration}@ = *__begin;
...
\begin{bnf}
	...
	\added{\terminal{coreturn-keyword} expression\opt \terminal{;}}\br
  \added{\terminal{coreturn-keyword} braced-init-list \terminal{;}}\br
  \added{\terminal{yield_kw} expression \terminal{;}}\br
	\added{\terminal{yield_kw} braced-init-list \terminal{;}}\br
...
\rSec2[stmt.coreturn]{The \tcode{coreturn-keyword} statement}%
...
\pnum
A coroutine returns to its caller by the \tcode{coreturn-keyword} statement
or when suspended at a suspend-resume point (\ref{dcl.fct.def.coroutine}). A \tcode{coreturn-keyword} statement shall not appear in a function other than a coroutine.
...
\pnum
A \tcode{coreturn-keyword} statement
with neither an \grammarterm{expression} nor a \grammarterm{braced-init-list}
...
A \tcode{coreturn-keyword} statement with an expression of non-\tcode{void} type can be used only
...
\pnum
A \tcode{coreturn-keyword} statement with an expression of type \cvvoid{ }can be used only in functions without an eventual return type 
...
Let \textit{yielded value} be the operand of the \tcode{yield_kw} statement and \textit{p} be the promise object of the enclosing coroutine.
If the result type of \tcode{\textit{p}.yield_value(\textit{yielded-value})} is of type \cvvoid, then the \tcode{yield_kw} statement is equivalent to:
...
%\pnum
%A \tcode{yield_kw} statement may only appear if a \tcode{yield_value} member
...
%      coreturn-keyword;
%    
%    yield_kw flatten(n->left);
%    yield_kw n->value;
%    yield_kw flatten(n->right);
%  }
%\end{codeblock}
%
