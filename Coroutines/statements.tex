...
\begin{bnf}
	...
	\added{\terminal{coreturn_kw} expression\opt \terminal{;}}\br
  \added{\terminal{coreturn_kw} braced-init-list \terminal{;}}\br
...
\rSec2[stmt.coreturn]{The \tcode{coreturn_kw} statement}%
...
\pnum
A coroutine returns to its caller by the \tcode{coreturn_kw} statement
or when suspended at a suspend-resume point (\ref{dcl.fct.def.coroutine}). A \tcode{coreturn_kw} statement shall not appear in a function other than a coroutine.
...
A \tcode{coreturn_kw} statement with an expression of non-\tcode{void} type can be used only
...
\pnum
A \tcode{coreturn_kw} statement with an expression of type \cvvoid{ }can be used only in functions without an eventual return type 
