...
\pnum
A \tcode{for await} statement may only appear in a coroutine function 
with an eventual return type, i.e a coroutine function shall have the \tcode{return_value} or \tcode{return_void} member
functions defined in its \term{promise type} (\ref{dcl.fct.def.coroutine}).
...
%\added{A coroutine function also returns to its caller 
%when suspended at suspend-resume point.}
%
%\pnum
%\added{In a non-coroutine function a}\removed{A} return statement
...
%\indextext{\idxcode{return}!constructor~and}%
%\added{In a coroutine function a  return statement
%	with neither an \grammarterm{expression} nor a \grammarterm{braced-init-list}
%	can be used only in functions
...
%\added{or coroutine functions returning an eventual value}; the value of the expression is returned
...
%\added{non-coroutine} function \added{or in an eventual-value-returning coroutine function}.
%
%\pnum
...
%\cvqual{cv} \tcode{void} \added{or coroutine functions with eventual return type of \tcode{void}}; 
...
%\setcounter{Paras}{3}
%\pnum 
%In a coroutine function return statement is replaced with
%a call to __pr.set_result, where _p
%\end{quote}

Add sections \ref{stmt.return.coroutine} and \ref{stmt.yield}.

\rSec2[stmt.return.coroutine]{The \tcode{return} statement in coroutine functions}%
%\enternote
%In this section function refers to coroutine function only.
%The return statement in non-coroutine functions is described in \ref{stmt.return}
%\exitnote

\pnum
A coroutine function returns to its caller by the \tcode{return} statement
or when suspended at a \term{suspend-resume point} (\ref{dcl.fct.def.coroutine}).
...
that do not produce an eventual value or have an eventual return type of \tcode{void}. In the latter case, completion of the coroutine function
is reported by calling \tcode{_Pr.return_void()}.
...
\tcode{_Pr.return_value(\grammarterm{expression})} or
\tcode{_Pr.return_value(\grammarterm{braced-init-list})}.
...
\tcode{true}, the coroutine function suspends at \term{final suspend point} (\ref{dcl.fct.def.coroutine}),
otherwise, coroutine function flows off the end of the function-body and destroys the \term{coroutine function state} and frees any extra memory dynamically allocated to store the state.

%\enternote
%If coroutine function instance lifetime is controlled 
%by a RAII object, it is expected that \tcode{_Pr.final_suspend()} returns true
%and coroutine function state will be destroyed when
%destructor of an owner object runs.
%
%For detached tasks, where lifetime of the coroutine function ends
...
Where \tcode{_Pr} is a \term{promise object} (\ref{dcl.fct.def.coroutine}) of the enclosing coroutine function.
