...
\added{A suspension of a coroutine (\ref{expr.await}) is not considered to be an exit from the scope.}
...
or when suspended (\ref{expr.await}). \cbstart\added{A coroutine shall not return to its caller or resumer by the \tcode{return} statement (\ref{stmt.return}).}\cbend
...
where $final$\tcode{\_}$suspend$ is as defined in \ref{dcl.fct.def.coroutine} and $S$ is an expression defined as follows:
...
\pnum 
\cbstart
\removed{\enternote See \ref{dcl.fct.def.coroutine} about the flowing off the end of a coroutine. \exitnote}

\added{If $p$}\tcode{\added{.return_void()}} \added{is a valid expression, flowing off the end of a coroutine is equivalent to a }\tcode{\added{co_return}} \added{with no operand; otherwise flowing off the end of a coroutine results in undefined behavior.}
\cbend
