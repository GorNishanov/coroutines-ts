...
\pnum
A \tcode{for await} statement may only appear in the resumable functions 
...
true, resumable function suspends at \term{final suspend point} (\ref{dcl.fct.def.resumable}),
otherwise, resumable function flows off the end of the function-body and destroys \term{resumable funciton state} and frees any extra memory dynamically allocated to store the state.
...
where \tcode{V} is either \grammarterm{expression} or \grammarterm{braced-init-list} is equivalent to:
...
Promise for flatten function should contain overloads that can accept a value of type \tcode{int} and a value of type \tcode{recursive_generator<int>}.
In the case of a former, yielding a value is unconditional. In the case of the latter, nested generator may produce an empty sequence of values and thus suspension at the yield point shall not happen and corresponding \tcode{yield_value} must return false. 
\exitexample