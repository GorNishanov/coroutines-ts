...
with an eventual return type, i.e a resumable function shall have the \tcode{return_value} or \tcode{return_void} member
functions defined in its \term{promise type} (\ref{dcl.fct.def.resumable}).
...
Flowing off the end of a function is equivalent to a \tcode{return} with
no value; 
this results in an implementation-defined behavior in an eventual-value-returning resumable function.
\enternote
