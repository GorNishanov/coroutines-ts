...
\tcode{return_void} and \tcode{return_value} are looked up in the scope of class $P$. 
If both are found, the program is ill-formed. Flowing off the end of a coroutine is equivalent to a \mbox{\tcode{co_return}} with no operand; this results in
undefined behavior if the expression \mbox{$p$\tcode{.return_void()}} is ill-formed.
