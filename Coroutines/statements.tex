...
\begin{bnf}
	...
	\terminal{for} \added{\terminal{co_await}\opt} \terminal{(} for-range-declaration \terminal{:} for-range-initializer \terminal{)} statement\br
...
\begin{ncbnf}
  \terminal{for}  \added{\terminal{co_await}\opt} \terminal{(} for-range-declaration : expression \terminal{)} statement
\end{ncbnf}
...
\begin{ncbnf}
  \terminal{for} \added{\terminal{co_await}\opt} \terminal{(} for-range-declaration \terminal{:} braced-init-list \terminal{)} statement
\end{ncbnf}
...
where 
\added{\tcode{co_await} appears if and only if it appears immediately after the \tcode{for} keyword, and}\linebreak
...
\begin{quote}
  \begin{bnf}
    ...
    \added{\terminal{co_return} expression\opt \terminal{;}}\br
    \added{\terminal{co_return} braced-init-list \terminal{;}}\br
...
\rSec3[stmt.return.coroutine]{The return statement of a coroutine}%
...
or when suspended at a suspend-resume point. 

\pnum
Let $P$ be a coroutine promise type and $p$ is a coroutine promise object (\ref{dcl.fct.def.coroutine}), 
...
\begin{itemize}
  \item A coroutine return statement
  with no operand shall be used only in coroutines
  without an eventual return type in which case $S$ is nothing and in coroutine with eventual return type of \tcode{void} in which case $S$ is $p$\tcode{.return_void();}
  
  \item A coroutine return statement with an operand of type \tcode{void} shall be used only in functions without an eventual return type 
  or with an eventual return type of \tcode{void}; in the former case, $S$ is $operand$; in the latter case, $S$ is
\begin{codeblock}
  @$operand$@; @p@.return_void();
\end{codeblock}
  
  \item A coroutine return statement with any other operand shall be used only in coroutines with non-\tcode{void} eventual type. In this case $S$ is
  
  \begin{codeblock}
    @p@.return_value(@$operand$@);
  \end{codeblock}
  where $operand$ is treated as an xvalue.
\end{itemize}
