...
one or more suspend-resume-points introduced by a potentially-evaluated \grammarterm{await-expression} (\ref{expr.await}) and a \tcode{yield_kw} statement (\ref{stmt.yield}).
...
%add.reference to [over.match.funcs]
...
std::experimental::generator<char> hello_fn() {
  for (auto ch: "Hello, world") yield_kw ch;
}
  
int main() {
  auto hello_lambda = []{ for (auto ch: "Hello, world") yield_kw ch; };
