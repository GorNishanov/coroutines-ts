...
\pnum
For a coroutine function \tcode{f}, Let \tcode{R} be a return type and $P_1$, $P_2$, ..., $P_n$
...
a coroutine function and its type is a \defn{promise type}
of the coroutine function.
%\pnum
An execution of a coroutine function is suspended when it reaches a suspend-resume point.

\pnum 
A \defn{suspension} of a coroutine function returns control to the current
...
of the \term{promise object} of the coroutine function.
...
\pnum
An invocation of a coroutine function may incur a move operation for the parameters that may be accessed in the \grammarterm{function-body}
