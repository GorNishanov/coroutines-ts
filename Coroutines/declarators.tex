...
%}
of \textit{f}, \textit{T} be the type \tcode{std::coroutine_traits<$R$,$P_1$,...,$P_n$>}, and \textit{P} be the type denoted by \textit{T::}\tcode{promise_type}. 
...
\end{codeblock}
where the local variable \textit{p} is 
defined for
exposition only and
\textit{F'} is \textit{F}
if \textit{P} does not declare a \tcode{set_exception} member function, 
and 
\begin{codeblock}
