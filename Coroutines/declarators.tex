...
one or more suspend-resume-points introduced by a potentially-evaluated \tcode{coawait} operator (\ref{expr.await}) expression and a \tcode{coyield} statement (\ref{stmt.yield}).
...
the \tcode{destroy} member function (\ref{coroutine.handle.resumption}) of an object of \tcode{std::experimental::coroutine_handle<\textit{P}}> associated with that coroutine is invoked.
...
%NOTE: massage
\pnum
The region of a coroutine state storing  parameters to the coroutine is copy-initialized with the parameter values considered as xvalues. 
