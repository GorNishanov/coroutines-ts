...
\pnum
A \defn{Suspend-resume point} is introduced by a potenitally-evaluated \tcode{coawait} operator (\ref{expr.await}) expression,
a \tcode{coyield} statement (\ref{stmt.yield}), 
% implicit NOTE: including implicit
or a \tcode{cofor} statement (\ref{stmt.for.await}). Every coroutine
...
\pnum
For a coroutine \textit{f}, let \tcode{R} be the return type and $P_1$, $P_2$, ..., $P_n$
...
\end{codeblock}
where the type alias \tcode{_Tr} and the local variable \tcode{_Pr} are 
defined for
exposition only and
\textit{stop-or-propagate} is \tcode{throw} 
if \tcode{_Tr::promise_type} does not declare a \tcode{set_exception} member function, and \tcode{_Pr.set_exception(std::current_exception())} otherwise. An object denoted as \tcode{_Pr} is the \defn{promise object} of
a \textit{f} and its type is a \defn{promise type}
of the coroutine.

%NOTE: not expand this in code
%The promise type of f is the type of.
%The promise object p is an object of promise type
%promise type metavariable
%do promise for the type, dodges typename
...
the \tcode{destroy} member function (\ref{coroutine.handle.resumption}) of \tcode{coroutine_handle} object associated with that function is invoked.
