...
\textit{F'} is \textit{F} 
%unless the \grammarterm{unqualified-id} \tcode{set_exception} is looked up in the scope of class \textit{P}
%as if by class member access lookup (\cxxref{basic.lookup.classref}), and if a declaration was found, then \textit{F'} is
unless the \grammarterm{unqualified-id} \tcode{set_exception} is found in the scope of \textit{P}
...
the coroutine and type $P$ is a \defn{promise type}
of the coroutine.
