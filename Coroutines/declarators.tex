...
\tcode{$p$.final_suspend(std::experimental::coroutine_handle<$P$>::from_promise($p$))} is evaluated.
The result of this expression must have type \tcode{std::experimental::coroutine_handle<}$Z$\tcode{>} for some type $Z$.
The coroutine identified by the returned handle is resumed as if by calling the \tcode{resume()} method. 
\enternote Any number of coroutines may be successively resumed in this fashion, eventually returning control flow to the current coroutine caller or resumer (\ref{dcl.fct.def.coroutine}). \exitnote
...
\pnum
The coroutine state is destroyed when the \tcode{destroy} member function (\ref{coroutine.handle.resumption})
of an object of type \tcode{std::experimental::coroutine_handle<\textit{P}}> associated with this coroutine is invoked.
...
The storage for the coroutine state is released by calling a non-array deallocation function~(\cxxref{basic.stc.dynamic.deallocation}).
If \tcode{destroy} is called for a coroutine that is not suspended, the program has undefined behavior.
