...
one or more suspend-resume-points introduced by a potentially-evaluated \grammarterm{await-expression} (\ref{expr.await}) and a \tcode{yield-keyword} statement (\ref{stmt.yield}).
...
\pnum
The deallocation function's name is looked up in the scope of the coroutine promise type. If this lookup fails to find the name, the deallocation function's name is looked up in the global scope.
...
\pnum
If type \textit{P} defines static member function \tcode{get_return_object_on_allocation_failure} (\ref{coroutine.traits}), then \tcode{std::nothrow_t} forms of allocation and deallocation functions will be used and the result of the allocation function call needs to be compared with \tcode{nullptr}, and if it is \tcode{nullptr}, coroutine must return control 
to the current caller of the coroutine and the return value
is obtained by a call to \tcode{P::get_return_object_on_allocation_failure()}.
