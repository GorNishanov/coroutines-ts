...
\begin{quote}
\pnum
A function is a \defn{coroutine} if it contains a \grammarterm{coroutine-return-statement} or
a suspend-resume-point introduced by an \grammarterm{await-expression} (\ref{expr.await}), a \grammarterm{yield-expression} (\ref{expr.yield}), or a \grammarterm{range-based-for} (\ref{stmt.ranged}).
