...
\pnum
For a coroutine \textit{f} that is a non-static member function, let $P_1$ denote the type of the implicit object parameter (\cxxref{over.match.funcs}) and $P_2$ ... $P_n$ be the types of the function parameters; otherwise let $P_1$ ... $P_n$ be the types of the function parameters. \cbstart \added{Let $p_1$ ... $p_n$ be lvalues denoting those objects.} \cbend
...
\begin{codeblock}
  {
     @\textit{P }$p$@;
     @\cbstart\removed{auto gro = p.get_return_object();}\cbend@
     co_await @$p$@.initial_suspend(); // initial suspend point
     @\cbstart\added{\tcode{try \{ \textit{F} \} catch(...) \{ \textit{p}.unhandled_exception(); \} }}\cbend@
  @\textit{final_suspend}@:
...
where \cbstart
\removed{\textit{F'} is}
\removed{\tcode{try \{ \textit{F} \} catch(...) \{ \textit{p}.unhandled_exception(); \}}
...
An}\cbend \added{an} object denoted as \textit{p} is the \defn{promise object} of
...
\pnum 
When a coroutine returns to its caller, the return value is produced
\cbstart
\removed{When a coroutine returns to its caller, the return value is produced as if by the statement \mbox{\tcode{return}{ }$gro$\tcode{;}}}
\added{by a call to \linebreak
...
\pnum
\cbstart
An implementation may need to allocate additional storage for \removed{the lifetime of} a coroutine. 
\cbend
...
\tcode{nullptr} if it fails to obtain storage, and if a global allocation function is selected, \cbstart \added{the} \cbend \tcode{::operator new(size_t, nothrow_t)} form shall be used.
...
\cbstart \added{The allocation function used in this case must have a non-throwing \grammarterm{noexcept-specification}.}
\cbend
...
\cbstart
\added{The selected deallocation function shall be called with the address of the block of storage to be reclaimed as its first argument. If a deallocation function with a parameter of type \tcode{std::size_t} is used, the size of the block is passed as the corresponding argument.}
\cbend
...
\cbstart
\removed{Parameter copies scope is a block scope of the coroutine promise.}
\added{Parameter copies lifetime ends immediately after the coroutine promise object lifetime ends.}
\cbend
%\pnum
...
struct generator {
  @\cbstart\added{struct promise_type;}\cbend@
