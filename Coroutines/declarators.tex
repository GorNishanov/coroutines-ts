...
\pnum
A function is considered \defn{resumable} if it contains
...
be types of parameters. If \tcode{f} is a non-static member function then $P_1$ denotes the type of the implicit this parameter. 
Resumable traits for function \tcode{f} is an insantiation of
...
\end{codeblock}
where type alias \tcode{_Tr} and local variable \tcode{_Pr} are for
exposition only and
\textit{stop-or-propate} is \tcode{throw} 
if promise_type does not have \tcode{set_exception} member function, and \tcode{_Pr.set_exception(std::current_exception())} otherwise. An object denoted as \tcode{_Pr} is a \defn{promise object} of
...
\pnum
An invocation of a resumable function may incur a copy or move operation for the parameters that may be accessed in the \grammarterm{function-body}
of resumable function after a resume. These copies
are defined in the same scope as \term{promise object}. 
References to parameters are replaced to 
references to their copies in the \grammarterm{function-body}
of the resumable function.
