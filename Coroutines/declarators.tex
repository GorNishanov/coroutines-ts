...
\pnum
If the evaluation of the expression $p$\tcode{.unhandled_exception()} exits via an exception,
the coroutine is considered suspended at the final suspend point.

\pnum
The expression \tcode{co_await} $p$\tcode{.final_suspend()} shall not be potentially-throwing (\cxxref{except.spec}).
