...
\pnum 
When a coroutine reaches \textit{initial suspend point}, the coroutine is suspended and execution returns
to the caller a value produced by a call to \linebreak
\mbox{\tcode{$p$.get_return_object(std::experimental::coroutine_handle<\textit{P}>::from_promise(p))}}. \linebreak
