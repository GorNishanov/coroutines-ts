...
by class member access lookup (\cxxref{basic.lookup.classref}). If a declaration is found, then the result of a call to an allocation function used to obtain storage for the coroutine state is assumed to return \tcode{nullptr} if it fails to obtain storage and; if a global allocation function is selected, \tcode{::operator new(size_t, nothrow_t)} form shall be used.
