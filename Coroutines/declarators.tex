...
\begin{quote}
\pnum
A function is a \defn{coroutine} if it contains a \grammarterm{coroutine-return-statement},  an \grammarterm{await-expression} (\ref{expr.await}), a \grammarterm{yield-expression} (\ref{expr.yield}), or a \grammarterm{range-based-for} (\ref{stmt.ranged}) with \tcode{co_await}.
The \grammarterm{parameter-declaration-clause} of the coroutine shall not terminates with an ellipsis that is not part of an \grammarterm{abstract-declarator}. 
...
\end{codeblock}
where %local variable $p$ is the \textit{promise object} of the coroutine $f$; 
\textit{F'} is \textit{F} 
%unless the unqualified-id \tcode{set_exception} is looked up in the scope of class \textit{P}
%as if by class member access lookup (\cxxref{basic.lookup.classref}), and if a declaration was found, then \textit{F'} is
unless the unqualified-id \tcode{set_exception} is found in the scope of \textit{P}
as if by class member access lookup (\cxxref{basic.lookup.classref}), then \textit{F'} is
...
No header needs to be included for this use of the  \tcode{std::current_exception}.
...
associated with this instance of the coroutine, where $P$
is the promise type of the coroutine. The function that invoked a resumption member function is called \term{resumer}. Invoking resumption member functions for a coroutine that is not suspended results in undefined behavior. 
