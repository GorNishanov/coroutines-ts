...
\end{codeblock}
where 
%the local variable \textit{p} is defined for exposition only and
\textit{F'} is \textit{F} unless the unqualified-id \tcode{set_exception} is looked up in the scope of class \textit{P}
...
%Implicit \grammarterm{await-expression}{}s introducing initial and final suspend points are not considered \grammarterm{await-expression}{}s for the purpose of type deduction (\ref{dcl.spec.auto}). 
...
a resumption member function (\ref{coroutine.handle.resumption}) of an object of \tcode{coroutine_handle<$P$>} type
...
%If type \textit{P} defines static member function \tcode{get_return_object_on_allocation_failure} (\ref{coroutine.traits}), 
 then \tcode{std::nothrow_t} forms of allocation and deallocation functions will be used. If an allocation function returns \tcode{nullptr}, coroutine must return control to the caller of the coroutine and the return value shall be obtained by a call to \tcode{P::get_return_object_on_allocation_failure()}.
