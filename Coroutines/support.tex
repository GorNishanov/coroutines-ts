...
%  returns an allocator (\cxxref{allocator.requirements}) that the implementation shall use
%  to dynamically allocate memory for coroutine state if dynamic allocation is required.
%  If \tcode{get_allocator} is not present, 
%  the implementation shall use \tcode{allocator<char>}.
%\\ \rowsep
%  \tcode{X::get_return_object_on_allocation_failure()}     &
%\textit{(optional)} If present, it is assumed that an
%allocator's \tcode{allocate} function will return \tcode{nullptr} in case of an
%allocation failure. If a coroutine requires dynamic allocation, it must check if an \tcode{allocate} returns \tcode{nullptr}, and if so it shall use the expression \tcode{X::get_return_object_on_allocation_failure()} to construct the return value and return back to the caller.
% 
\\ 
\end{concepttable}
...
\begin{itemdescr}
  ...
  at the final suspend point, \tcode{terminate} is called (\cxxref{except.terminate}).
\end{itemdescr}
...
\begin{itemdescr}
  ...
  \pnum
  \effects destroys the target coroutine (\ref{dcl.fct.def.coroutine}).
\end{itemdescr}
...
\begin{libreqtab2}
	...
	\tcode{h}       &   a value of \tcode{experimental::coroutine_handle<P>} type    \\ \rowsep
...
a \tcode{return} statement, or flowing off the end of the function.
	\\ \rowsep
	\tcode{P::get_return_object_on_allocation_failure()}        &
