...
Add underlined text to paragraph 2.
...
\pnum The header \tcode{<coroutine>} defines
...
denotes a type (\cxxref{temp.deduct}),
then
...
\pnum Program defined specializations of this template shall define a publicly accessible nested type named \tcode{promise_type}.
...
on an execution agent other than the one it was suspended on has implementation-defined
behavior unless each is either an instance of \tcode{std::thread} or the thread that executes \tcode{main}.

\enternote A coroutine that is resumed on a different execution agent should avoid relying on consistent
thread identity throughout, such as holding a mutex object across a suspend point.
\exitnote

\enternote A concurrent resumption of the coroutine may result in a data race.
\exitnote
...
\begin{itemdescr}
  \pnum
  \returns \tcode{less<>()(x.address(), y.address())}.
\end{itemdescr}
...
\begin{itemdescr}
  ...
  \pnum
  \remarks A \tcode{noop_coroutine_handle}'s \tcode{ptr} is always a non-\tcode{null} pointer value.
\end{itemdescr}
...
The header \tcode{<coroutine>} defines \tcode{suspend_never} and \tcode{suspend_always} as follows.
\begin{codeblock}
namespace std {
  struct suspend_never {
