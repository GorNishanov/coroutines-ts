...
if \tcode{p.initial_suspend()} is evaluates to \tcode{true}, resumable function will suspend at \textit{initial suspend point} (\ref{dcl.fct.def.resumable}).
	   \\ \rowsep
	\tcode{p.final_suspend()}     &  
if \tcode{p.final_suspend()} is evaluates to \tcode{true}, resumable function will suspend at \textit{final suspend point} (\ref{dcl.fct.def.resumable}).
\\ 
\end{concepttable}
