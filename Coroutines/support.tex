...
\begin{concepttable}{\tcode{resumable_traits} requirements}{tab:resumable.traits.requirements}
  ...
  \tcode{X::promise_type}     &
  \tcode{X::promise_type} must be a type satisfying resumable promise requirements (\ref{resumable.promise}) that completes the definition
  of resumable function semantics.
  \\ \rowsep
...
allocation failure. A resumable function requires dynamic allocation, it must check if an \tcode{allocate} returns nullptr, and if so it shall use the expression \tcode{X::get_return_object_on_allocation_failure()} to construct the return value and return back to the caller.
  \\ 
\end{concepttable}
...
discovered via instantiation of \tcode{resumable_traits}.
...
eventual value of type that \tcode{v} can be converted to. \tcode{set_result} is invoked by
...
\footnotetext{A promise type must contain at most one declaration of \tcode{set_result}.}
	\\ \rowsep
...
if \tcode{p.initial_suspend()} is evaluates to \tcode{true}, resumable function will suspend at initial suspend point prior to entering \grammarterm{function-body}.
	   \\ \rowsep
	\tcode{p.final_suspend()}     &  
if \tcode{p.final_suspend()} is evaluates to \tcode{true}, resumable function will suspend at final suspend point. 
\\ 
\end{concepttable}
