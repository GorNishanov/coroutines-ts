...
\begin{libsumtab}{Language support library summary}{tab:lang.sup.lib.summary}
  ...
  & \added{Coroutine functions support} 
...
\rSec1[support.coroutine]{Coroutine functions support library}
...
$a_1$, $a_2$, ... $a_n$ denote parameters passed to a coroutine function. If it is a member function, than $a_1$ denotes implicit \tcode{this} parameter.
...
%  that completes the definition of coroutine function semantics.
  \\ \rowsep
...
allocation failure. If a coroutine function requires dynamic allocation, it must check if an \tcode{allocate} returns nullptr, and if so it shall use the expression \tcode{X::get_return_object_on_allocation_failure()} to construct the return value and return back to the caller.
  \\ 
\end{concepttable}
...
\begin{itemdescr}
  \pnum
  \precondition \tcode{p} points to a promise object of a coroutine function.
...
\tcode{get_return_object} is invoked by the coroutine function to construct the
...
statement in the body results in a compile time error.
	\\ \rowsep
	\tcode{p.return_void()}     &   
If present, an enclosing coroutine function supports an eventual value of type \tcode{void}. \tcode{return_void} is invoked by
a coroutine function when 
a return statement
...
\tcode{set_exception} is invoked by a coroutine function when an
...
Must be present for the enclosing coroutine function to support \tcode{yield} statement.
	\\ \rowsep
	\tcode{p.initial_suspend()}     &
if \tcode{p.initial_suspend()} evaluates to \tcode{true}, coroutine function will suspend at \textit{initial suspend point} (\ref{dcl.fct.def.coroutine}).
	   \\ \rowsep
	\tcode{p.final_suspend()}     &  
if \tcode{p.final_suspend()} evaluates to \tcode{true}, coroutine function will suspend at \textit{final suspend point} (\ref{dcl.fct.def.coroutine}).
\\ 
\end{concepttable}
