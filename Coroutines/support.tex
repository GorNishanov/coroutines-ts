...
to customize an allocation policy and a promise type for resumable functions with particular set of signatures.
...
allocation failure. A resumable function must check if an \tcode{allocate} returns nullptr, and if so it shall use the expression \tcode{X::get_return_object_on_allocation_failure()} to construct the return value and return back to the caller.
  \\ 
\end{concepttable}
