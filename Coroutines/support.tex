...
inline namespace coroutines_v1 {
  ...
  template <typename R, typename... ArgTypes>
    @\cbstart\added{struct}\removed{class}\cbend@ coroutine_traits;
  	
  // \ref{coroutine.handle} coroutine handle
  template <typename Promise = void>
    @\cbstart\added{struct}\removed{class}\cbend@ coroutine_handle;		
...
template <class P> struct hash<@\added{std::experimental::}@coroutine_handle<P>>;@\cbend@
...
if \tcode{ArgTypes...} is a sequence of zero or more types and if \tcode{R} is a type that has a valid (\cxxref{temp.deduct}) member type \tcode{promise_type}, then
\tcode{coroutine_traits<R,ArgTypes...>} shall have the following publicly accessible member:

\begin{codeblock}
  using promise_type = typename R::promise_type;
\end{codeblock}

Otherwise, \tcode{coroutine_traits<R,ArgTypes...>} shall have no members.
\end{addedblock}
...
inline namespace coroutines_v1 {
  ...
  struct coroutine_handle<void>
  {
    ...
    @\cbstart\added{constexpr}@ void* address() const noexcept;
    @\added{constexpr}\cbend@ static coroutine_handle from_address(void* addr) noexcept;		
    
    // \ref{coroutine.handle.capacity} capacity
    @\cbstart\added{constexpr}\cbend@ explicit operator bool() const noexcept;
    ...
    bool done() const @\cbstart\removed{noexcept}\cbend@; 
  @\cbstart\added{private:}@
    @\added{void* ptr;}\cbend@ // exposition only
  };
  ...
  struct coroutine_handle : coroutine_handle<>
  {
    ...
    @\added{constexpr static coroutine_handle from_address(void* addr) noexcept;}\cbend@
    
    // \ref{coroutine.handle.prom} promise access
    Promise& promise() const @\cbstart\removed{noexcept}\cbend@;		
  };
...
\begin{itemdecl}
  @\cbstart\added{constexpr}\cbend@ static coroutine_handle from_address(void* addr) noexcept;		
  @\cbstart\added{constexpr}\cbend@ void* address() const noexcept;
\end{itemdecl}
...
\begin{itemdecl}
  Promise& promise()  @\cbstart\removed{noexcept}\cbend@;		
  Promise const& promise() const  @\cbstart\removed{noexcept}\cbend@;
\end{itemdecl}
...
inline namespace coroutines_v1 {
  ...
  struct suspend_never {
    bool await_ready() @\cbstart\added{const}@ { return true; }
    void await_suspend(coroutine_handle<>) @\added{const}@ {}
    void await_resume() @\added{const}\cbend@ {}
  };
  struct suspend_always {
    bool await_ready() @\cbstart\added{const}@ { return false; }
    void await_suspend(coroutine_handle<>) @\added{const}@ {}
    void await_resume() @\added{const}\cbend@ {}
  };
