...
statement in the body results in a compile time error.
	\\ \rowsep
	\tcode{p.return_void()}     &   
If present, an enclosing coroutine supports an eventual value of type \tcode{void}. \tcode{return_void} is invoked by
a coroutine when 
a return statement
...
Must be present for the enclosing coroutine to support \tcode{yield} statement.
	\\ \rowsep
